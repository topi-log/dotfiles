...
ローカルで動かす開発環境を登録しておき、起動状況とポートを見ながら
起動・停止する。`denv` で一覧が開く。
...
`a` を押すと名前・ディレクトリ・起動コマンド・停止コマンドを順に聞かれる。
ポートは登録せず、起動したプロセスが実際に LISTEN しているものを表示する。

同じ操作は `denv start <name>` のようにコマンドでもできる。
残りのキーと詳細は `denv help` か [denv](docs/denv.md) を参照。
